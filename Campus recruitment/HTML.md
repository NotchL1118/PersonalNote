...
- src: **src常用作“拿取(引入)”，表示对资源的引入，src会将其指向的资源下载并应用到文档中。**如果是script的src引入，会阻碍文档的解析，如果是图片的引入，并不会阻碍文档的解析。
- href: **href常用作“链接前往(引用)”，它指向一些网络资源，建立资源和当前元素的链接关系。**当浏览器遇到href会并行下载资源并且不会停止对当前文档的处理
