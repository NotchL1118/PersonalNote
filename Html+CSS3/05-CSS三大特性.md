...
**元素如果有浏览器默认样式，继承性仍然存在，但**

1. a标签的color会继承失效
2. h系列标签的`font-size`会继承失败
...
`span`父级为`<p class="son>"`，也就是继承了`div p` 所以最终颜色为红色，如果`<p class="son">`加了一个id选择器，则就跟随id选择器了，因为id选择器优先级比类选择器高
