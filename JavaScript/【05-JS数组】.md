...
// 从下标为 3 的项开始，连续删除 2 项，并追加'X','Y','Z'。即将 'D', 'E' 替换为 'X', 'Y', 'Z'
...
思路2：准备一个空的对象，遍历原数组，将原数组的值加入到空对象中，相同的值会互相覆盖，最后将对象的键输出出来
```javascript
var arr = [1, 1, 1, 2, 2, 3, 3, 3, 2, 1];
var temp = {}
for（var i = 0; i < arr.length;i++)
...
for(var i in temp)
{
	res.push(i*1) //因为i会成字符串
}
```

思路3：运用`Set` 和 `Array.from`
```javaScript
var arr = [1, 1, 1, 2, 2, 3, 3, 3, 2, 1];
var set = new Set(arr)
...
![](F:\Obsidian\JavaScript\assets\5c4ac9feb8ce4e3289acb75376e667b8.gif)
...
![Sorting_quicksort_anim](F:\Obsidian\JavaScript\assets\Sorting_quicksort_anim.gif)
...
<img src="F:\Obsidian\JavaScript\assets\bg2011040403.png" alt="img" style="zoom:50%;" />

第二步，按照顺序，将每个元素与 "基准" 进行比较，形成两个子集，一个 "小于45"，另一个 "大于等于45"。

<img src="F:\Obsidian\JavaScript\assets\bg2011040404.png" alt="img" style="zoom:50%;" />

第三步，对两个子集不断重复第一步和第二步，直到所有子集只剩下一个元素为止。

<img src="F:\Obsidian\JavaScript\assets\bg2011040405.png" alt="img" style="zoom:50%;" />

<img src="F:\Obsidian\JavaScript\assets\bg2011040406.png" alt="img" style="zoom:50%;" />

<img src="F:\Obsidian\JavaScript\assets\bg2011040407.png" alt="img" style="zoom:50%;" />

<img src="F:\Obsidian\JavaScript\assets\bg2011040408.png" alt="img" style="zoom:50%;" />
...
arr.forEach(function(item,index,arr){
    console.log(item,idex,arr) //第一个参数是数组内容，第二个参数是元素索引值，第三个是整个数组
})
```
...
console.log(arr2) //["<li>aa<li>","<li>bb<li>","<li>cc<li>"]
```

## 9.16 filter()过滤

按照返回条件，返回值为 `true` 的条件会加入到新数组，并返回一个满足条件的新数组
...
每一个都判断一下条件，只要有一个不满足条件就返回`false`

```javascript
var arr = [80,90,100]
var arr = arr.every(function(item){
    return item > 90
})
...
```javascript
var arr = [80,90,100]
var arr = arr.some(function(item){
    return item > 90
})
...
```javascript
var arr = [{name:"AA",grade: 80},{name:"BB",grade:100}]
var arr2 = arr.find(function(item){
    return item.grade === 100
})
console.log(arr2) //{name:"BB",grade:100}
```

注意：和`filter` 不同的就是，`filter` 返回的是一个数组.find只返回第一个符合条件的元素
...
*/
```
基于初始值开始叠加，上一次的结果会送到下一次的prev一直到最后一个元素
# 十、二维数组
...
<img src="F:\Obsidian\JavaScript\assets\78d2f78a614042ef998a91c992154d7e.png" style="zoom:50%;" />

<img src="F:\Obsidian\JavaScript\assets\9dfcfceed30440c290efc2ec3226919b.png" style="zoom:50%;" />
